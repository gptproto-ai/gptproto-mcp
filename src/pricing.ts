...
function capability(modelName: string, tags: readonly string[]): PriceCapability {
  const values = [modelName.split("/").slice(1).join("/"), ...tags].map((value) => value.toLowerCase());
  if (values.some((value) => value.includes("-to-video") || value === "motion-control" || value === "start-end-frame")) return "video";
  if (values.some((value) => value.includes("-to-image") || value === "image-edit")) return "image";
  if (values.some((value) => value.includes("audio") || value === "voice-clone")) return "audio";
  if (values.some((value) => value.includes("-to-3d"))) return "3d";
  if (values.some((value) => value.includes("-to-text") || value === "file-analysis" || value === "web-search")) return "text";
  return "other";
}

function normalizeCapability(value: string): PriceCapability {
  const normalized = value.trim().toLowerCase().replace(/s$/, "");
  if (["text", "image", "video", "audio", "3d", "other"].includes(normalized)) return normalized as PriceCapability;
...
function normalizeItem(value: unknown): ModelPrice | undefined {
  ...
  const starting = (["fixed", "input", "output", "cache_write", "cache_read"] as const)
  ...
  return {
    ...
    pricing: {
      ...
      starting_price: rates[starting] as string,
...
async function fetchCatalog(config: Pick<MCPConfig, "timeoutMs">, language: string): Promise<ModelPrice[]> {
  ...
  try {
    ...
    if (!response.ok) throw new MCPError(`Price catalog request failed with HTTP ${response.status}`, response.status, body);
    if (!Array.isArray(body.data?.items)) throw new MCPError("Price catalog response does not contain data.items");
    return body.data.items.map(normalizeItem).filter((item): item is ModelPrice => item !== undefined);
  } catch (error) {
    if (error instanceof MCPError) throw error;
    if (error instanceof DOMException && error.name === "AbortError") throw new MCPError(`Price catalog request timed out after ${config.timeoutMs} ms`);
...
export async function queryPrices(config: Pick<MCPConfig, "timeoutMs">, query: PriceQuery = {}): Promise<PriceSearchResult> {
  const language = query.language?.trim() || "en";
  let models = await fetchCatalog(config, language);
  if (query.model) models = models.filter((item) => item.id.toLowerCase() === query.model?.trim().toLowerCase());
  if (query.capability) models = models.filter((item) => item.capability === normalizeCapability(query.capability as string));
  if (query.mode?.trim()) {
  ...
  if (query.sort === "price") models.sort((left, right) => Number(left.pricing.starting_price) - Number(right.pricing.starting_price) || left.id.localeCompare(right.id));
  if (query.sort === "name") models.sort((left, right) => left.id.localeCompare(right.id));
  if (query.limit !== undefined) {
    if (!Number.isInteger(query.limit) || query.limit <= 0) throw new MCPError("limit must be a positive integer");
