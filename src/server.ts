...
import { TaskRegistry, type StoredTask } from "./tasks.js";
...
function success(value: unknown, text?: string) {
  return {
    content: [{ type: "text" as const, text: text ?? jsonText(value) }],
...
function callResult(result: ClientCallResult, outputJson = false, taskId?: string) {
  if (Buffer.isBuffer(result.response.body)) {
    ...
    if (mimeType.startsWith("audio/")) {
      return {
        content: [{ type: "audio" as const, data: result.response.body.toString("base64"), mimeType }],
        structuredContent: { ...(taskId ? { task_id: taskId } : {}), content_type: mimeType, bytes: result.response.body.length },
      };
    }
    if (mimeType.startsWith("image/")) {
      return {
        content: [{ type: "image" as const, data: result.response.body.toString("base64"), mimeType }],
        structuredContent: { ...(taskId ? { task_id: taskId } : {}), content_type: mimeType, bytes: result.response.body.length },
      };
    }
    return failure(new Error("Binary API responses are not returned by this MCP tool"));
  }
  if (outputJson) return success(result.response.body);
  const extracted = extractApiResult(result.response.body, result.route.output);
  if (extracted.type === "error") {
    return {
      content: [{ type: "text" as const, text: extracted.error ?? "GPTProto request failed" }],
...
type StringValues = Record<string, string | string[]>;

function pairs(value: StringValues | undefined): [string, string][] | undefined {
  if (!value) return undefined;
  return Object.entries(value).flatMap(([name, entry]) =>
    Array.isArray(entry) ? entry.map((item): [string, string] => [name, item]) : [[name, entry]],
  );
}

function requestModel(request: Pick<ClientRequest, "body" | "form">): string | undefined {
  if (request.body && typeof request.body === "object" && !Array.isArray(request.body)) {
    const model = (request.body as Record<string, unknown>).model;
    if (typeof model === "string" && model) return model;
  }
  return request.form?.find(([name]) => name === "model")?.[1];
}

function inferredResource(result: ClientCallResult, explicit?: string): string | undefined {
  if (explicit) return explicit;
  const value = `${result.route.family} ${result.route.path}`.toLowerCase();
  if (value.includes("image")) return "image";
  if (value.includes("video") || value.includes("veo") || value.includes("sora")) return "video";
  if (value.includes("audio") || value.includes("speech") || value.includes("transcription")) return "audio";
  if (result.route.output === "text" || /responses|messages|chat|completions|generatecontent/.test(value)) return "text";
  return undefined;
}

function updateStoredTask(client: GPTProtoClient, tasks: TaskRegistry, id: string, result: ClientCallResult): void {
...
function registerTask(
  ...
  resource?: string,
  label?: string,
  describedPollingPath?: string,
): { id?: string; pollingPath?: string } {
  const snapshot = taskSnapshot(result.response.body);
  if (!snapshot.id) return {};
  if (!snapshot.pollingUrl && result.route.async !== true && !resource) return {};
...
async function pollingPathBeforeCreate(client: GPTProtoClient, request: ClientRequest): Promise<string | undefined> {
  if (findPublicRoute(request.method, request.path)?.async !== true) return undefined;
  try {
...
async function storedPollingPath(
  client: GPTProtoClient,
  task: StoredTask | undefined,
): Promise<string | undefined> {
  if (!task) return undefined;
  if (task.polling_path) return task.polling_path;
  if (!task.model) return undefined;
  try {
    const described = await client.describeInvocation({
      method: task.method as ClientRequest["method"],
      path: task.path,
...
export function createServer(client = new GPTProtoClient(), tasks = new TaskRegistry()): McpServer {
  const server = new McpServer(
    { name: "gptproto", version: VERSION },
    {
      instructions:
        "Discover models before calling them. Use gptproto_models_list, then gptproto_model_describe. " +
  ...
  server.registerTool(
    ...
    async ({ capability }) => {
      try { return success(await client.listModels(capability)); } catch (error) { return failure(error); }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ model }) => {
      try { return success(await client.describeModel(model)); } catch (error) { return failure(error); }
    },
  );
  ...
  server.registerTool(
    ...
    async (query) => {
      try {
        ...
        if (query.model && result.models.length === 0) return failure(new Error(`No price was found for ${query.model}`));
        ...
        if (tracked.id) {
          ...
          result = await client.waitForCall(result, request, {
            ...
            onUpdate: async (update) => {
              updateStoredTask(client, tasks, tracked.id as string, update);
              await notifyTaskProgress(
                context,
                tracked.id as string,
                startedAt,
                taskSnapshot(update.response.body).status,
              );
            },
          });
        }
  ...
  server.registerTool(
    ...
    async ({ task_id, video, path, output_json }) => {
      try {
        ...
        if (stored) updateStoredTask(client, tasks, task_id, result);
