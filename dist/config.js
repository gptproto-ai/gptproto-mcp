...
export function readConfig() {
    const baseUrl = validateBaseUrl(process.env.GPTPROTO_API_BASE_URL?.trim() || stored(URL_FILE) || DEFAULT_API_BASE_URL);
    ...
    return {
        ...
        maxPollMs: positiveInteger("GPTPROTO_MAX_POLL_SECONDS", process.env.GPTPROTO_MAX_POLL_SECONDS
            ? `${Number(process.env.GPTPROTO_MAX_POLL_SECONDS) * 1000}`
            : undefined, DEFAULT_MAX_POLL_MS),
        allowFileUploads: booleanFlag("GPTPROTO_MCP_ALLOW_FILE_UPLOADS", process.env.GPTPROTO_MCP_ALLOW_FILE_UPLOADS),
    };
}
