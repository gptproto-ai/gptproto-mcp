...
function pathRegex(template) {
    const pattern = template.split(/(\{[^}]+\})/g).map((part) => /^\{[^}]+\}$/.test(part) ? "[^/:]+" : part.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("");
    return new RegExp(`^${pattern}$`);
}
export function matchesPathTemplate(template, path) {
    return pathRegex(template).test(path.split("?", 1)[0]);
}
export function findPublicRoute(method, path) {
